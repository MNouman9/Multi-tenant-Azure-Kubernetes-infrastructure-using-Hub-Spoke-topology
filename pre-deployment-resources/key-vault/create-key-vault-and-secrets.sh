...
# Variables
keyVaultName="kv-devops-vars"
keyVaultResourceGroupName="rg-azuredevops-pipeline-terraform"
keyVaultSku="Standard"
location="EastUS"
storageAccountName="tfazuredevopspipeline"
storageAccountResourceGroupName="rg-azuredevops-pipeline-terraform"
...
# Check if the secret already exists
terraformBackendSubscriptionNameSecretName="terraformBackendSubscriptionName"
terraformBackendSubscriptionNameSecretValue=$subscriptionName

echo "Checking if [$terraformBackendSubscriptionNameSecretName] secret actually exists in the [$keyVaultName] key vault..."

az keyvault secret show --name $terraformBackendSubscriptionNameSecretName --vault-name $keyVaultName &> /dev/null

if [[ $? != 0 ]]; then
	echo "No [$terraformBackendSubscriptionNameSecretName] secret actually exists in the [$keyVaultName] key vault"
    echo "Creating [$terraformBackendSubscriptionNameSecretName] secret in the [$keyVaultName] key vault..."
    
    # Create the secret
    az keyvault secret set \
    --name $terraformBackendSubscriptionNameSecretName \
    --vault-name $keyVaultName \
    --value "$terraformBackendSubscriptionNameSecretValue" 1> /dev/null
        
    if [[ $? == 0 ]]; then
        echo "[$terraformBackendSubscriptionNameSecretName] secret successfully created in the [$keyVaultName] key vault"
    else
        echo "Failed to create [$terraformBackendSubscriptionNameSecretName] secret in the [$keyVaultName] key vault"
        exit
    fi
else
	echo "[$terraformBackendSubscriptionNameSecretName] secret already exists in the [$keyVaultName] key vault"
fi
