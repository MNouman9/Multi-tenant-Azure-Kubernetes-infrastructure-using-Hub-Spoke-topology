...
output "aks_cluster_client_id" {
  value       = azurerm_user_assigned_identity.aks_identity.principal_id
  description = "Specifies the client id of the service principal for the AKS cluster."
}
...
output "ingress_application_gateway_identity" {
   description = "ingress_application_gateway_identity"
   value       = azurerm_kubernetes_cluster.aks_cluster.ingress_application_gateway[0].ingress_application_gateway_identity[0]
}
