...
variable windows_profile_enabled {
  description = "(Optional) Should the windows_profile block be defined."
  type        = bool
  default     = false
}

variable "outbound_nat_enabled" {
  description = "(Optional) Should the Windows nodes in this Node Pool have outbound NAT enabled? Defaults to true. Changing this forces a new resource to be created."
  type          = bool
  default       = false
}